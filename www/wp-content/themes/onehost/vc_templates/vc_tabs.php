<?php
$output = $title = $interval = $el_class = $icon_position = '';
...
foreach ( $tab_titles as $tab ) {
	...
	if(isset($tab_atts['title'])) {
		$icon = isset( $tab_atts['icon'] ) && $tab_atts['icon'] ? '<i class="' . $tab_atts['icon'] . '"></i>' : '';
		$tabs_nav .= '<li><a href="#tab-' . ( isset( $tab_atts['tab_id'] ) ? $tab_atts['tab_id'] : sanitize_title( $tab_atts['title'] ) ) . '">' . $icon . '<span>' . $tab_atts['title'] . '</span>' . '</a></li>';
	}
}
$tabs_nav .= '</ul>' . "\n";

$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, trim( $element . ' wpb_content_element ' . $el_class . ' icon-position-' . $icon_position ), $this->settings['base'], $atts );
