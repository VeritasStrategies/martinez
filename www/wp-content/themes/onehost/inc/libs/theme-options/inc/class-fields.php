...
class TA_Theme_Options_Fields {
	...
	function image( $field, $value ) {
		...
		if ( ! $multiple ) {
			return $this->tag( 'div', $attr,
				...
				$this->tag( 'button', array( 'type' => 'button', 'class' => 'button button-primary button-select' ), __( 'Upload Image', 'onehost' ) ),
				$this->tag( 'i', array( 'class' => 'entypo-squared-minus button-clear', 'title' => __( 'Remove Image', 'onehost' ) ) ),
		...
		return $this->tag( 'div', $attr,
			...
			$this->tag( 'button', array( 'type' => 'button', 'class' => 'button button-primary button-select' ), __( 'Upload Images', 'onehost' ) )
		);
	}
	...
	function font( $field, $value ) {
		...
		$fonts = array( '' => __( 'Select', 'onehost' ) );
		if ( isset( $field['standard'] ) ) {
			$fonts[] = array(
				'label'   => __( 'Standard Fonts', 'onehost' ),
				'options' => $field['standard']
			);
		}
		if ( isset( $field['google'] ) ) {
			$fonts[] = array(
				'label'   => __( 'Google Fonts', 'onehost' ),
		...
		return $this->tag( 'div', array( 'class' => 'horizontal' ),
			$this->element( array(
				'desc'    => __( 'Size', 'onehost' ),
				'type'    => 'number',
				'name'    => $field['name'] . '[size]',
				'suffix'  => 'px',
				'subdesc' => __( 'Size', 'onehost' ),
			), $value['size'] ),
			$this->element( array(
				'desc'    => __( 'Font Family', 'onehost' ),
				'type'    => 'select',
				'name'    => $field['name'] . '[font]',
				'subdesc' => __( 'Family', 'onehost' ),
				'options' => $fonts,
			), $value['font'] ),
			$this->element( array(
				'desc'     => __( 'Style', 'onehost' ),
				'type'     => 'toggle',
				'multiple' => true,
				'name'     => $field['name'] . '[styles]',
				'subdesc'  => __( 'Style', 'onehost' ),
				'options'  => array( 'bold' => '<strong>B</strong>', 'italic' => '<i>I</i>', 'underline' => '<u>U</u>' ),
			), $value['styles'] ),
			$this->element( array(
				'desc'    => __( 'Color', 'onehost' ),
				'type'    => 'color',
				'name'    => $field['name'] . '[color]',
				'subdesc' => __( 'Color', 'onehost' ),
			), $value['color'] )
		);
	}
	...
	function background( $field, $value ) {
		...
		$ui = $this->tag( 'div', array( 'class' => 'horizontal' ),
			$this->tag(
				...
				$this->tag( 'button', array( 'type' => 'button', 'class' => 'button button-primary button-select' ), __( 'Upload Image', 'onehost' ) ),
				$this->tag( 'button', array( 'type' => 'button', 'class' => 'button button-clear' ), __( 'Remove Image', 'onehost' ) ),
			...
			$this->tag( 'div', array( 'class' => 'vertical' ),
				$this->tag( 'div', array( 'class' => 'horizontal background-position' ),
					$this->element( array(
						...
						'options' => array(
							'left'   => __( 'Left', 'onehost' ),
							'center' => __( 'Center', 'onehost' ),
							'right'  => __( 'Right', 'onehost' )
						)
					), $value['position_x'] ),
					$this->element( array(
						'type'    => 'select',
				...
				$this->element( array(
					...
					'options' => array(
						'repeat'    => __( 'Repeat', 'onehost' ),
						'repeat-x'  => __( 'Repeat Horizontally', 'onehost' ),
						'repeat-y'  => __( 'Repeat Vertically', 'onehost' ),
						'no-repeat' => __( 'No Repeat', 'onehost' ),
					)
				), $value['repeat'] ),
				$this->element( array(
					'type'    => 'select',
					...
					'options' => array(
						'scroll' => __( 'Scroll', 'onehost' ),
						'fixed'  => __( 'Fixed', 'onehost' ),
					)
				), $value['attachment'] ),
				$this->element(
					array(
	...
	function social( $field, $value ) {
		$socials = array(
			'facebook'   => __( 'Facebook', 'onehost' ),
			'twitter'    => __( 'Twitter', 'onehost' ),
			'google'     => __( 'Google', 'onehost' ),
			'tumblr'     => __( 'Tumblr', 'onehost' ),
			'flickr'     => __( 'Flickr', 'onehost' ),
			'vimeo'      => __( 'Vimeo', 'onehost' ),
			'youtube'    => __( 'Youtube', 'onehost' ),
			'linkedin'   => __( 'LinkedIn', 'onehost' ),
			'pinterest'  => __( 'Pinterest', 'onehost' ),
			'dribbble'   => __( 'Dribbble', 'onehost' ),
			'behance'    => __( 'Behance', 'onehost' ),
			'soundcloud' => __( 'SoundCloud', 'onehost' ),
			'spotify'    => __( 'Spotify', 'onehost' ),
			'github'     => __( 'GitHub', 'onehost' ),
			'instagram'  => __( 'Instagram', 'onehost' ),
			'picasa'     => __( 'Picasa', 'onehost' ),
			'spotify'    => __( 'Sportify', 'onehost' ),
			'foursquare' => __( 'Foursquare', 'onehost' ),
			'lastfm'     => __( 'Lastfm', 'onehost' ),
		);
		...
		foreach ( $socials as $name => $label ) {
			...
			$settings[] = $this->tag(
				...
				$this->tag( 'i', array( 'class' => 'deactive-social-link entypo-cross', 'title' => __( 'Remove', 'onehost' ) ) ),
				$this->element(
					array(
	...
	function backup( $field, $value ) {
		...
		$this->tag( 'button', array( 'class' => 'button import-options' ), __( 'Import Opitons', 'onehost' ) );
	}
